...
describe('ACL Validation Tests', () => {
  ...
  let adminUser: User;
  let editorUser1: User;
  let editorUser2: User;
  ...
  beforeAll(async () => {
    ...
    // Create test roles
    const roles = await TestSetup.createTestRoles(roleRepository);
    adminRole = roles.adminRole;
    editorRole = roles.editorRole;
    userRole = roles.userRole;

    // Create test users
    const users = await TestSetup.createTestUsers(userRepository, roles);
    adminUser = users.adminUser;
    regularUser = users.regularUser;

    // Create additional editor users for cross-user testing
    editorUser1 = userRepository.create({
      nome: 'Editor User 1',
      usuario: 'editor1',
      senha: await bcrypt.hash(TEST_CONSTANTS.DEFAULT_PASSWORD, 12),
      role: editorRole,
      ativo: true,
      tentativasLogin: 0,
    });
  ...
  describe('Role-based Access Control', () => {
    ...
    describe('Editor Role Permissions', () => {
      ...
      it('editor deve poder acessar apenas seus próprios registros', async () => {
        ...
        // Não pode acessar registro de outro editor
  ...
  describe('Record-level Access Control', () => {
    let testRecord: Desarquivamento;
    ...
    describe('canBeEditedBy method', () => {
      ...
      it('deve negar edição de registros concluídos por não-admin', async () => {
        testRecord.status = StatusDesarquivamento.CONCLUIDO;
        await desarquivamentoRepository.save(testRecord);
    ...
    describe('canBeDeletedBy method', () => {
      ...
      it('deve negar deleção de registros em andamento por não-admin', async () => {
        testRecord.status = StatusDesarquivamento.EM_ANDAMENTO;
        await desarquivamentoRepository.save(testRecord);
  ...
  describe('Status-based Access Control', () => {
    let pendingRecord: Desarquivamento;
    let inProgressRecord: Desarquivamento;
    let completedRecord: Desarquivamento;
    let cancelledRecord: Desarquivamento;
    ...
    beforeEach(async () => {
      ...
      completedRecord = desarquivamentoRepository.create({
        ...
        status: StatusDesarquivamento.CONCLUIDO,
      });
      ...
      cancelledRecord = desarquivamentoRepository.create({
        ...
        status: StatusDesarquivamento.CANCELADO,
      });
    ...
    it('registros em andamento devem ter edição limitada', () => {
      ...
      expect(inProgressRecord.canBeDeletedBy(editorUser1)).toBe(false); // Não pode deletar em andamento
    });
  ...
  describe('User State Validation', () => {
    it('usuários bloqueados não devem ter acesso', async () => {
      const blockedUser = userRepository.create({
        ...
        role: editorRole,
        ativo: true,
        tentativasLogin: 5, // Máximo de tentativas
  ...
  describe('Cross-user Access Scenarios', () => {
    it('deve validar cenários complexos de acesso cruzado', async () => {
      // Criar registros de diferentes usuários
      ...
      // Editor1 deve acessar apenas seus próprios
      ...
      // Editor2 deve acessar apenas seus próprios
      ...
      // Usuário regular não deve acessar nenhum
