...
export class CreateDesarquivamentosTable1700000004000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_NUMERO_NIC', columnNames: ['numero_nic_laudo_auto'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_STATUS', columnNames: ['status'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_TIPO', columnNames: ['tipo_desarquivamento'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_DATA_SOLICITACAO', columnNames: ['data_solicitacao'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_CREATED_BY', columnNames: ['created_by'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_RESPONSAVEL', columnNames: ['responsavel_id'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_URGENTE', columnNames: ['urgente'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_DELETED_AT', columnNames: ['deleted_at'] })
    );

    await queryRunner.createIndex(
      'desarquivamentos',
      new TableIndex({ name: 'IDX_DESARQUIVAMENTOS_SETOR', columnNames: ['setor_demandante'] })
    );
  }
